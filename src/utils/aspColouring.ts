...
export function colourAspPage(request: AspColouringRequest): AspColouringResult {
    ...
    const lineSource: LineSource = { lineCount: lines.length, lineAt: line => ({ text: lines[line] }) };
        ...
        if (quoteCol !== -1) {
            ...
            if (endsWithContinuation && (hasAmpersandBeforeUnderscore || isPlainContinuation)) {
                for (let scanLi = li + 1; scanLi < lineCount; scanLi++) {
                    ...
                    if (scanQuote !== -1) {
                        const group = extractSqlGroup(lineSource, scanLi, scanQuote);
                        if (group !== null) {
                            stitchedValue = group.stitched;
                            for (const seg of group.segments) { processedAssignLines.add(seg.lineIndex); }
                        } else {
    ...
    for (const fn of allSymbols.functions) {
        ...
        for (let li = fn.line; li <= fn.endLine; li++) {
            ...
            const group = extractSqlGroup(lineSource, quoteLi, quoteCol);
            let stitched = '';
            if (group !== null) {
    ...
    for (const [varName3, assignments] of assignmentMap) {
        ...
        for (const a of assignments) {
            ...
            for (let li = 0; li < lineCount; li++) {
                ...
                warnings.push({
                    line: li, character: varCol, length: varName3.length,
                    message: `'${am3[1]}' is already marked as a SQL variable. Highlighting may be incorrect.`,
                });
                break;
            }
        }
        ...
        while ((m2 = identPattern.exec(rhsText)) !== null) {
            ...
            if (isCall) {
                ...
                if (nonStrFuncs.has(wordKey)) {
                    warnings.push({
                        line: li, character: col, length: word.length,
                        message: `'${word}()' does not appear to return a string. ` +
                                 `Concatenating it into a SQL variable may produce unexpected results.`,
                    });
                } else {
                    warnings.push({
                        line: li, character: col, length: word.length,
                        message: `'${word}()' is concatenated into SQL variable '${assignMatch[1]}' ` +
    ...
    for (let li = 0; li < lineCount; li++) {
        ...
        while (col < lineText.length) {
            ...
            const group = extractSqlGroup(lineSource, li, col);
