 ...
 */
export const FUNCTION = /((?:^[\t ]*'+.*$(?:\r\n|\n))*)^[\t ]*((?:(?:Public|Private)[\t ]+)?(Function|Sub)[\t ]+((\[?[a-z]\w*\]?)[\t ]*(?:\((.*)\))?))/gim;
 ...
 */
export const CLASS = /((?:^[\t ]*'+.*$(?:\r\n|\n))*)^[\t ]*((?:(?:Public|Private)[\t ]+)?Class[\t ]+(\[?[a-z]\w*\]?))/gim;
 ...
 * 5. Params
 */
export const PROP = /((?:^[\t ]*'+.*$(?:\r\n|\n))*)^[\t ]*((?:Public[\t ]+(?:Default[\t ]+)?|Private[\t ]+)?Property[\t ]+(Get|Let|Set)[\t ]+(\[?[a-z]\w*\]?))(?:\((.*)\))?/gim;
...
export const VAR = /(?<!'\s*)(?:^|:)[\t ]*(Dim|Set|Const|Private[\t ]+Const|Public[\t ]+Const|Private|Public)[\t ]+(?!Sub|Function|Property)([a-z0-9_]+(?:[\t ]*\([\t ]*\d*[\t ]*\))?(?:[\t ]*,[\t ]*[a-z0-9_]+(?:[\t ]*\([\t ]*\d*[\t ]*\))?)*)[\t ]*.*(?:$|:)/gim;
 ...
 * 3. Name
 */
export function DEF(input: string, word: string): RegExpExecArray | null {
    return new RegExp(
...
export function DEFVAR(input: string, word: string): RegExpExecArray | null {
    return new RegExp(
 ...
 */
export const COMMENT_SUMMARY = /(?:\s*<summary>\s*)([^<]*)(?:<\/summary>)?/gim;
 ...
 */
export const PARAM_SUMMARIES =
    /(?:\s*<param name=["'](\w+)["'].*?>\s*)([^<]*)(?:<\/param>)?/gim;

export function PARAM_SUMMARY(
    input: string,
    parameterName: string,
): RegExpExecArray | null {
    return new RegExp(
        `'''\\s*<param name=["']${parameterName}["']>(.*)<\\/param>`,
        "i",
    ).exec(input);
}
...
export const COLOR =
    /\b(vbBlack|vbBlue|vbCyan|vbGreen|vbMagenta|vbRed|vbWhite|vbYellow)\b|\b(RGB[\t ]*\([\t ]*(&h[0-9a-f]+|\d+)[\t ]*,[\t ]*(&h[0-9a-f]+|\d+)[\t ]*,[\t ]*(&h[0-9a-f]+|\d+)[\t ]*\))|(&h[0-9a-f]{6}\b)/gi;
